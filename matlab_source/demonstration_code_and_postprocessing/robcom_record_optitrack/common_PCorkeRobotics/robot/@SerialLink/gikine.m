...
function [qt,histout] = gikine(robot, tr, varargin)
    ...
    if gflag
        r.L     = [robot.links(1).a      robot.links(2).a] ;
        r.alpha = [robot.links(1).alpha  robot.links(2).alpha] ;
        r.d     = [robot.links(1).d      robot.links(2).d] ;
        J0      = analyticalRobotJacobian( r, q  );
    else
        J0      = jacob0(robot, q);
    end
    J0 = J0(m, m);
    ...
    for i=1:npoints
        ...
        while true
            ...
            if gflag
                T_tmp = FK_and_homog_transf( r.L, r.alpha, r.d, q);
                T_tmp = T_tmp{end};
                e = tr2delta( T_tmp, T);
            else
