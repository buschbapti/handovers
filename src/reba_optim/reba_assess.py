...
class RebaAssess(object):
    def __init__(self, window_size=1, save_score=False):
    ...
    def from_joints_to_reba(self, joints, names):
        ...
        # trunk
        data["trunk"] = []
        data["trunk"].append(joints[names.index("spine_1")]) # flexion
        data["trunk"].append(joints[names.index("spine_0")]) # bend
        data["trunk"].append(joints[names.index("spine_2")]) # twist
        # neck
        data["neck"] = []
        data["neck"].append(joints[names.index("neck_1")]) # flexion
        data["neck"].append(joints[names.index("neck_0")]) # bend
        data["neck"].append(joints[names.index("neck_2")]) # twist
        # legs
        data["leg_R"] = [1, joints[names.index("right_knee")]] # TODO check ground contact
        data["leg_L"] = [1, joints[names.index("left_knee")]] # TODO check ground contact
        # shoulders
        data["shoulder_R"] = []
        data["shoulder_R"].append(joints[names.index("right_shoulder_1")]-np.pi/2) # flexion
        data["shoulder_R"].append(joints[names.index("right_shoulder_0")]-np.pi/2) # abduction
        data["shoulder_R"].append(joints[names.index("right_shoulder_2")]) # twist
        data["shoulder_L"] = []
        data["shoulder_L"].append(joints[names.index("left_shoulder_1")]-np.pi/2) # flexion
        data["shoulder_L"].append(joints[names.index("left_shoulder_0")]+np.pi/2) # abduction
        data["shoulder_L"].append(joints[names.index("left_shoulder_2")]) # twist
        # elbows
        data["elbow_R"] = [names.index("right_elbow_0")]
        data["elbow_L"] = [names.index("left_elbow_0")]
        # wrist
        data["wrist_R"] = []
        data["wrist_R"].append(names.index("right_wrist_1")) # flexion
        data["wrist_R"].append(names.index("right_wrist_0")) # deviation
        data["wrist_R"].append(names.index("right_elbow_1")) # twist
        data["wrist_L"] = []
        data["wrist_L"].append(names.index("left_wrist_1")) # flexion
        data["wrist_L"].append(names.index("left_wrist_0")) # deviation
        data["wrist_L"].append(names.index("left_elbow_1")) # twist
        # misc
        data["load"] = 0
        data["static"] = 0
        data["high_dynamic"] = 0
    ...
    def reba_assess(self,data,epsilon=5):
        ...
        def get_B_score():
            ...
            elbows_score = 0
            wrists_score = 0
            # get maximum angle for both arms
